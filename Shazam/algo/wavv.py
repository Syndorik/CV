...
def wave2array(nchan,largech,data): #nchan = 1 (mono) 2 (stéréo) largech nb d'octet sur lequel un echantillon est codé, data retourne un tableau  où chaque élément est 1 octet de donnée(hexadecimal). Sachant qu'il y largech octet pour une donné et si on est en sétéréo il y a 2 echantillon pour un seul et même echantillongage ( gauche et droite) la formule de nb echantillon est bien le quotient de la div euclidienne de len(data) par largech*nchan
    ...
    if largech ==3:
        a = np.empty((nbechantillon, nchan, 4), dtype=np.uint8) #Création d'un tableau qui ne prend que des entiers entre0 et 255
        datatransfo=np.fromstring(data,dtype=np.uint8) #on passe de la base hexadecimal à la base 10, ce n'est plus une chaine de caractère mais des entiers compris entre 0 et 255 car 1 octet = 8 bits = 2**8-1 nombres positif, chaque octet est transformé
        ...
        result = a.view('<i4').reshape(a.shape[:-1]) # Prenons la gauche de la stéréo, <i4 permet de lire chaque quadruplet dont chaque élément représente 1 octet comme 
    else:
